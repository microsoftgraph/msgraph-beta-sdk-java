// ------------------------------------------------------------------------------
// Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.  See License in the project root for license information.
// ------------------------------------------------------------------------------

package com.microsoft.graph.requests.extensions;
import com.microsoft.graph.models.extensions.SalesCreditMemo;

import com.microsoft.graph.concurrency.ICallback;
import com.microsoft.graph.core.ClientException;
import com.microsoft.graph.http.IHttpRequest;

// **NOTE** This file was generated by a tool and any changes will be overwritten.

/**
 * The interface for the Sales Credit Memo Request.
 */
public interface ISalesCreditMemoRequest extends IHttpRequest {

    /**
     * Gets the SalesCreditMemo from the service
     *
     * @param callback the callback to be called after success or failure
     */
    void get(final ICallback<SalesCreditMemo> callback);

    /**
     * Gets the SalesCreditMemo from the service
     *
     * @return the SalesCreditMemo from the request
     * @throws ClientException this exception occurs if the request was unable to complete for any reason
     */
    SalesCreditMemo get() throws ClientException;

    /**
     * Delete this item from the service
     *
     * @param callback the callback when the deletion action has completed
     */
    void delete(final ICallback<SalesCreditMemo> callback);

    /**
     * Delete this item from the service
     *
     * @throws ClientException if there was an exception during the delete operation
     */
    void delete() throws ClientException;

    /**
     * Patches this SalesCreditMemo with a source
     *
     * @param sourceSalesCreditMemo the source object with updates
     * @param callback the callback to be called after success or failure
     */
    void patch(final SalesCreditMemo sourceSalesCreditMemo, final ICallback<SalesCreditMemo> callback);

    /**
     * Patches this SalesCreditMemo with a source
     *
     * @param sourceSalesCreditMemo the source object with updates
     * @return the updated SalesCreditMemo
     * @throws ClientException this exception occurs if the request was unable to complete for any reason
     */
    SalesCreditMemo patch(final SalesCreditMemo sourceSalesCreditMemo) throws ClientException;

    /**
     * Posts a SalesCreditMemo with a new object
     *
     * @param newSalesCreditMemo the new object to create
     * @param callback the callback to be called after success or failure
     */
    void post(final SalesCreditMemo newSalesCreditMemo, final ICallback<SalesCreditMemo> callback);

    /**
     * Posts a SalesCreditMemo with a new object
     *
     * @param newSalesCreditMemo the new object to create
     * @return the created SalesCreditMemo
     * @throws ClientException this exception occurs if the request was unable to complete for any reason
     */
    SalesCreditMemo post(final SalesCreditMemo newSalesCreditMemo) throws ClientException;

    /**
     * Posts a SalesCreditMemo with a new object
     *
     * @param newSalesCreditMemo the object to create/update
     * @param callback the callback to be called after success or failure
     */
    void put(final SalesCreditMemo newSalesCreditMemo, final ICallback<SalesCreditMemo> callback);

    /**
     * Posts a SalesCreditMemo with a new object
     *
     * @param newSalesCreditMemo the object to create/update
     * @return the created SalesCreditMemo
     * @throws ClientException this exception occurs if the request was unable to complete for any reason
     */
    SalesCreditMemo put(final SalesCreditMemo newSalesCreditMemo) throws ClientException;

    /**
     * Sets the select clause for the request
     *
     * @param value the select clause
     * @return the updated request
     */
    ISalesCreditMemoRequest select(final String value);

    /**
     * Sets the expand clause for the request
     *
     * @param value the expand clause
     * @return the updated request
     */
    ISalesCreditMemoRequest expand(final String value);

}

