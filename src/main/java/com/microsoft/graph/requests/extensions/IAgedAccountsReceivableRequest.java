// ------------------------------------------------------------------------------
// Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.  See License in the project root for license information.
// ------------------------------------------------------------------------------

package com.microsoft.graph.requests.extensions;
import com.microsoft.graph.models.extensions.AgedAccountsReceivable;

import com.microsoft.graph.concurrency.ICallback;
import com.microsoft.graph.core.ClientException;
import com.microsoft.graph.http.IHttpRequest;

// **NOTE** This file was generated by a tool and any changes will be overwritten.

/**
 * The interface for the Aged Accounts Receivable Request.
 */
public interface IAgedAccountsReceivableRequest extends IHttpRequest {

    /**
     * Gets the AgedAccountsReceivable from the service
     *
     * @param callback the callback to be called after success or failure
     */
    void get(final ICallback<AgedAccountsReceivable> callback);

    /**
     * Gets the AgedAccountsReceivable from the service
     *
     * @return the AgedAccountsReceivable from the request
     * @throws ClientException this exception occurs if the request was unable to complete for any reason
     */
    AgedAccountsReceivable get() throws ClientException;

    /**
     * Delete this item from the service
     *
     * @param callback the callback when the deletion action has completed
     */
    void delete(final ICallback<AgedAccountsReceivable> callback);

    /**
     * Delete this item from the service
     *
     * @throws ClientException if there was an exception during the delete operation
     */
    void delete() throws ClientException;

    /**
     * Patches this AgedAccountsReceivable with a source
     *
     * @param sourceAgedAccountsReceivable the source object with updates
     * @param callback the callback to be called after success or failure
     */
    void patch(final AgedAccountsReceivable sourceAgedAccountsReceivable, final ICallback<AgedAccountsReceivable> callback);

    /**
     * Patches this AgedAccountsReceivable with a source
     *
     * @param sourceAgedAccountsReceivable the source object with updates
     * @return the updated AgedAccountsReceivable
     * @throws ClientException this exception occurs if the request was unable to complete for any reason
     */
    AgedAccountsReceivable patch(final AgedAccountsReceivable sourceAgedAccountsReceivable) throws ClientException;

    /**
     * Posts a AgedAccountsReceivable with a new object
     *
     * @param newAgedAccountsReceivable the new object to create
     * @param callback the callback to be called after success or failure
     */
    void post(final AgedAccountsReceivable newAgedAccountsReceivable, final ICallback<AgedAccountsReceivable> callback);

    /**
     * Posts a AgedAccountsReceivable with a new object
     *
     * @param newAgedAccountsReceivable the new object to create
     * @return the created AgedAccountsReceivable
     * @throws ClientException this exception occurs if the request was unable to complete for any reason
     */
    AgedAccountsReceivable post(final AgedAccountsReceivable newAgedAccountsReceivable) throws ClientException;

    /**
     * Posts a AgedAccountsReceivable with a new object
     *
     * @param newAgedAccountsReceivable the object to create/update
     * @param callback the callback to be called after success or failure
     */
    void put(final AgedAccountsReceivable newAgedAccountsReceivable, final ICallback<AgedAccountsReceivable> callback);

    /**
     * Posts a AgedAccountsReceivable with a new object
     *
     * @param newAgedAccountsReceivable the object to create/update
     * @return the created AgedAccountsReceivable
     * @throws ClientException this exception occurs if the request was unable to complete for any reason
     */
    AgedAccountsReceivable put(final AgedAccountsReceivable newAgedAccountsReceivable) throws ClientException;

    /**
     * Sets the select clause for the request
     *
     * @param value the select clause
     * @return the updated request
     */
    IAgedAccountsReceivableRequest select(final String value);

    /**
     * Sets the expand clause for the request
     *
     * @param value the expand clause
     * @return the updated request
     */
    IAgedAccountsReceivableRequest expand(final String value);

}

