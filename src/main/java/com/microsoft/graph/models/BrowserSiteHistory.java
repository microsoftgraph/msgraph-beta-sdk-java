...
public class BrowserSiteHistory implements AdditionalDataHolder, Parsable {
    ...
    public Map<String, Consumer<ParseNode>> getFieldDeserializers() {
        ...
        return new HashMap<String, Consumer<ParseNode>>(8) {{
            this.put("allowRedirect", (n) -> { currentObject.setAllowRedirect(n.getBooleanValue()); });
            this.put("comment", (n) -> { currentObject.setComment(n.getStringValue()); });
            this.put("compatibilityMode", (n) -> { currentObject.setCompatibilityMode(n.getEnumValue(BrowserSiteCompatibilityMode.class)); });
            this.put("lastModifiedBy", (n) -> { currentObject.setLastModifiedBy(n.getObjectValue(IdentitySet::createFromDiscriminatorValue)); });
            this.put("mergeType", (n) -> { currentObject.setMergeType(n.getEnumValue(BrowserSiteMergeType.class)); });
            this.put("@odata.type", (n) -> { currentObject.setOdataType(n.getStringValue()); });
            this.put("publishedDateTime", (n) -> { currentObject.setPublishedDateTime(n.getOffsetDateTimeValue()); });
            this.put("targetEnvironment", (n) -> { currentObject.setTargetEnvironment(n.getEnumValue(BrowserSiteTargetEnvironment.class)); });
        }};
    }
    /**
