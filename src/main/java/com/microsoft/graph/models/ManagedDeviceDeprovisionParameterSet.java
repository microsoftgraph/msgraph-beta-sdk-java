// Template Source: BaseMethodParameterSet.java.tt
// ------------------------------------------------------------------------------
// Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.  See License in the project root for license information.
// ------------------------------------------------------------------------------

package com.microsoft.graph.models;


import com.google.gson.annotations.Expose;
import com.google.gson.annotations.SerializedName;
import javax.annotation.Nonnull;
import javax.annotation.Nullable;
import com.google.gson.JsonObject;
import java.util.EnumSet;
import java.util.ArrayList;

// **NOTE** This file was generated by a tool and any changes will be overwritten.

/**
 * The class for the Managed Device Deprovision Parameter Set.
 */
public class ManagedDeviceDeprovisionParameterSet {
    /**
     * The deprovision Reason.
     * 
     */
    @SerializedName(value = "deprovisionReason", alternate = {"DeprovisionReason"})
    @Expose
	@Nullable
    public String deprovisionReason;


    /**
     * Instiaciates a new ManagedDeviceDeprovisionParameterSet
     */
    public ManagedDeviceDeprovisionParameterSet() {}
    /**
     * Instiaciates a new ManagedDeviceDeprovisionParameterSet
     * @param builder builder bearing the parameters to initialize from
     */
    protected ManagedDeviceDeprovisionParameterSet(@Nonnull final ManagedDeviceDeprovisionParameterSetBuilder builder) {
        this.deprovisionReason = builder.deprovisionReason;
    }
    /**
     * Gets a new builder for the body
     * @return a new builder
     */
    @Nonnull
    public static ManagedDeviceDeprovisionParameterSetBuilder newBuilder() {
        return new ManagedDeviceDeprovisionParameterSetBuilder();
    }
    /**
     * Fluent builder for the ManagedDeviceDeprovisionParameterSet
     */
    public static final class ManagedDeviceDeprovisionParameterSetBuilder {
        /**
         * The deprovisionReason parameter value
         */
        @Nullable
        protected String deprovisionReason;
        /**
         * Sets the DeprovisionReason
         * @param val the value to set it to
         * @return the current builder object
         */
        @Nonnull
        public ManagedDeviceDeprovisionParameterSetBuilder withDeprovisionReason(@Nullable final String val) {
            this.deprovisionReason = val;
            return this;
        }
        /**
         * Instanciates a new ManagedDeviceDeprovisionParameterSetBuilder
         */
        @Nullable
        protected ManagedDeviceDeprovisionParameterSetBuilder(){}
        /**
         * Buils the resulting body object to be passed to the request
         * @return the body object to pass to the request
         */
        @Nonnull
        public ManagedDeviceDeprovisionParameterSet build() {
            return new ManagedDeviceDeprovisionParameterSet(this);
        }
    }
    /**
     * Gets the functions options from the properties that have been set
     * @return a list of function options for the request
     */
    @Nonnull
    public java.util.List<com.microsoft.graph.options.FunctionOption> getFunctionOptions() {
        final ArrayList<com.microsoft.graph.options.FunctionOption> result = new ArrayList<>();
        if(this.deprovisionReason != null) {
            result.add(new com.microsoft.graph.options.FunctionOption("deprovisionReason", deprovisionReason));
        }
        return result;
    }
}
