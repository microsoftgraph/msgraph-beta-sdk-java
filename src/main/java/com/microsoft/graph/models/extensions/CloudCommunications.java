...
public class CloudCommunications extends Entity implements IJsonBackedObject {
    ...
    public void setRawObject(final ISerializer serializer, final JsonObject json) {
        ...
        if (json.has("presences")) {
            final PresenceCollectionResponse response = new PresenceCollectionResponse();
            if (json.has("presences@odata.nextLink")) {
                response.nextLink = json.get("presences@odata.nextLink").getAsString();
            }

            final JsonObject[] sourceArray = serializer.deserializeObject(json.get("presences").toString(), JsonObject[].class);
            final Presence[] array = new Presence[sourceArray.length];
            for (int i = 0; i < sourceArray.length; i++) {
                array[i] = serializer.deserializeObject(sourceArray[i].toString(), Presence.class);
                array[i].setRawObject(serializer, sourceArray[i]);
            }
            response.value = Arrays.asList(array);
            presences = new PresenceCollectionPage(response, null);
        }
    }
}
