...
public class Group extends DirectoryObject implements IJsonBackedObject {
     ...
     * 
     */
    @SerializedName("unseenConversationsCount")
    @Expose
    ...
    public void setRawObject(final ISerializer serializer, final JsonObject json) {
        ...
        if (json.has("permissionGrants")) {
            final ResourceSpecificPermissionGrantCollectionResponse response = new ResourceSpecificPermissionGrantCollectionResponse();
            if (json.has("permissionGrants@odata.nextLink")) {
                response.nextLink = json.get("permissionGrants@odata.nextLink").getAsString();
            }

            final JsonObject[] sourceArray = serializer.deserializeObject(json.get("permissionGrants").toString(), JsonObject[].class);
            final ResourceSpecificPermissionGrant[] array = new ResourceSpecificPermissionGrant[sourceArray.length];
            for (int i = 0; i < sourceArray.length; i++) {
                array[i] = serializer.deserializeObject(sourceArray[i].toString(), ResourceSpecificPermissionGrant.class);
                array[i].setRawObject(serializer, sourceArray[i]);
            }
            response.value = Arrays.asList(array);
            permissionGrants = new ResourceSpecificPermissionGrantCollectionPage(response, null);
        }

        if (json.has("settings")) {
            final DirectorySettingCollectionResponse response = new DirectorySettingCollectionResponse();
