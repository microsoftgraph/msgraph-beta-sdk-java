...
public class PolicyRoot implements IJsonBackedObject {
    ...
    public void setRawObject(final ISerializer serializer, final JsonObject json) {
        ...
        if (json.has("authorizationPolicy")) {
            final AuthorizationPolicyCollectionResponse response = new AuthorizationPolicyCollectionResponse();
            if (json.has("authorizationPolicy@odata.nextLink")) {
                response.nextLink = json.get("authorizationPolicy@odata.nextLink").getAsString();
            }

            final JsonObject[] sourceArray = serializer.deserializeObject(json.get("authorizationPolicy").toString(), JsonObject[].class);
            final AuthorizationPolicy[] array = new AuthorizationPolicy[sourceArray.length];
            for (int i = 0; i < sourceArray.length; i++) {
                array[i] = serializer.deserializeObject(sourceArray[i].toString(), AuthorizationPolicy.class);
                array[i].setRawObject(serializer, sourceArray[i]);
            }
            response.value = Arrays.asList(array);
            authorizationPolicy = new AuthorizationPolicyCollectionPage(response, null);
        }
