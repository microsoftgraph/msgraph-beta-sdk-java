...
public class EducationRoot extends Entity implements IJsonBackedObject {
    ...
    public void setRawObject(final ISerializer serializer, final JsonObject json) {
        ...
        if (json.has("synchronizationProfiles")) {
            final EducationSynchronizationProfileCollectionResponse response = new EducationSynchronizationProfileCollectionResponse();
            if (json.has("synchronizationProfiles@odata.nextLink")) {
                response.nextLink = json.get("synchronizationProfiles@odata.nextLink").getAsString();
            }

            final JsonObject[] sourceArray = serializer.deserializeObject(json.get("synchronizationProfiles").toString(), JsonObject[].class);
            final EducationSynchronizationProfile[] array = new EducationSynchronizationProfile[sourceArray.length];
            for (int i = 0; i < sourceArray.length; i++) {
                array[i] = serializer.deserializeObject(sourceArray[i].toString(), EducationSynchronizationProfile.class);
                array[i].setRawObject(serializer, sourceArray[i]);
            }
            response.value = Arrays.asList(array);
            synchronizationProfiles = new EducationSynchronizationProfileCollectionPage(response, null);
        }
