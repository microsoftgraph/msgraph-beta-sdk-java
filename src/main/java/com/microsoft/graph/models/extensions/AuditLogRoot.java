...
public class AuditLogRoot extends Entity implements IJsonBackedObject {
     ...
     * 
     */
    @SerializedName("directoryProvisioning")
    @Expose
    ...
    public void setRawObject(final ISerializer serializer, final JsonObject json) {
        ...
        if (json.has("directoryProvisioning")) {
            final ProvisioningObjectSummaryCollectionResponse response = new ProvisioningObjectSummaryCollectionResponse();
            if (json.has("directoryProvisioning@odata.nextLink")) {
                response.nextLink = json.get("directoryProvisioning@odata.nextLink").getAsString();
            }

            final JsonObject[] sourceArray = serializer.deserializeObject(json.get("directoryProvisioning").toString(), JsonObject[].class);
            final ProvisioningObjectSummary[] array = new ProvisioningObjectSummary[sourceArray.length];
            for (int i = 0; i < sourceArray.length; i++) {
                array[i] = serializer.deserializeObject(sourceArray[i].toString(), ProvisioningObjectSummary.class);
                array[i].setRawObject(serializer, sourceArray[i]);
            }
            response.value = Arrays.asList(array);
            directoryProvisioning = new ProvisioningObjectSummaryCollectionPage(response, null);
        }

        if (json.has("provisioning")) {
            final ProvisioningObjectSummaryCollectionResponse response = new ProvisioningObjectSummaryCollectionResponse();
            if (json.has("provisioning@odata.nextLink")) {
                response.nextLink = json.get("provisioning@odata.nextLink").getAsString();
            }

            final JsonObject[] sourceArray = serializer.deserializeObject(json.get("provisioning").toString(), JsonObject[].class);
            final ProvisioningObjectSummary[] array = new ProvisioningObjectSummary[sourceArray.length];
            for (int i = 0; i < sourceArray.length; i++) {
                array[i] = serializer.deserializeObject(sourceArray[i].toString(), ProvisioningObjectSummary.class);
                array[i].setRawObject(serializer, sourceArray[i]);
