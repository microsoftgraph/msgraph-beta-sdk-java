...
public class Device extends DirectoryObject implements IJsonBackedObject {
     ...
     * 
     */
    @SerializedName("deviceOwnership")
    @Expose
     ...
     * 
     */
    @SerializedName("enrollmentProfileName")
    @Expose
    ...
    public void setRawObject(final ISerializer serializer, final JsonObject json) {
        ...
        if (json.has("commands")) {
            final CommandCollectionResponse response = new CommandCollectionResponse();
            if (json.has("commands@odata.nextLink")) {
                response.nextLink = json.get("commands@odata.nextLink").getAsString();
            }

            final JsonObject[] sourceArray = serializer.deserializeObject(json.get("commands").toString(), JsonObject[].class);
            final Command[] array = new Command[sourceArray.length];
            for (int i = 0; i < sourceArray.length; i++) {
                array[i] = serializer.deserializeObject(sourceArray[i].toString(), Command.class);
                array[i].setRawObject(serializer, sourceArray[i]);
            }
            response.value = Arrays.asList(array);
            commands = new CommandCollectionPage(response, null);
        }
    }
}
