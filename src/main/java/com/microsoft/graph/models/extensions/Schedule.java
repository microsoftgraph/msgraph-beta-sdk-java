...
import com.microsoft.graph.models.extensions.TimeClockSettings;
...
public class Schedule extends Entity implements IJsonBackedObject {
     ...
     * 
     */
    @SerializedName("timeClockSettings")
    @Expose
    ...
    public void setRawObject(final ISerializer serializer, final JsonObject json) {
        ...
        if (json.has("timeCards")) {
            final TimeCardCollectionResponse response = new TimeCardCollectionResponse();
            if (json.has("timeCards@odata.nextLink")) {
                response.nextLink = json.get("timeCards@odata.nextLink").getAsString();
            }

            final JsonObject[] sourceArray = serializer.deserializeObject(json.get("timeCards").toString(), JsonObject[].class);
            final TimeCard[] array = new TimeCard[sourceArray.length];
            for (int i = 0; i < sourceArray.length; i++) {
                array[i] = serializer.deserializeObject(sourceArray[i].toString(), TimeCard.class);
                array[i].setRawObject(serializer, sourceArray[i]);
            }
            response.value = Arrays.asList(array);
            timeCards = new TimeCardCollectionPage(response, null);
        }
