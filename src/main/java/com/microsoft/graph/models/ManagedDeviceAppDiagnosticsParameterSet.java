// Template Source: BaseMethodParameterSet.java.tt
// ------------------------------------------------------------------------------
// Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.  See License in the project root for license information.
// ------------------------------------------------------------------------------

package com.microsoft.graph.models;

import com.microsoft.graph.models.PowerliftIncidentMetadata;
import com.google.gson.annotations.Expose;
import com.google.gson.annotations.SerializedName;
import javax.annotation.Nonnull;
import javax.annotation.Nullable;
import com.google.gson.JsonObject;
import java.util.EnumSet;
import java.util.ArrayList;

// **NOTE** This file was generated by a tool and any changes will be overwritten.

/**
 * The class for the Managed Device App Diagnostics Parameter Set.
 */
public class ManagedDeviceAppDiagnosticsParameterSet {
    /**
     * The upn.
     * 
     */
    @SerializedName(value = "upn", alternate = {"Upn"})
    @Expose
	@Nullable
    public String upn;


    /**
     * Instiaciates a new ManagedDeviceAppDiagnosticsParameterSet
     */
    public ManagedDeviceAppDiagnosticsParameterSet() {}
    /**
     * Instiaciates a new ManagedDeviceAppDiagnosticsParameterSet
     * @param builder builder bearing the parameters to initialize from
     */
    protected ManagedDeviceAppDiagnosticsParameterSet(@Nonnull final ManagedDeviceAppDiagnosticsParameterSetBuilder builder) {
        this.upn = builder.upn;
    }
    /**
     * Gets a new builder for the body
     * @return a new builder
     */
    @Nonnull
    public static ManagedDeviceAppDiagnosticsParameterSetBuilder newBuilder() {
        return new ManagedDeviceAppDiagnosticsParameterSetBuilder();
    }
    /**
     * Fluent builder for the ManagedDeviceAppDiagnosticsParameterSet
     */
    public static final class ManagedDeviceAppDiagnosticsParameterSetBuilder {
        /**
         * The upn parameter value
         */
        @Nullable
        protected String upn;
        /**
         * Sets the Upn
         * @param val the value to set it to
         * @return the current builder object
         */
        @Nonnull
        public ManagedDeviceAppDiagnosticsParameterSetBuilder withUpn(@Nullable final String val) {
            this.upn = val;
            return this;
        }
        /**
         * Instanciates a new ManagedDeviceAppDiagnosticsParameterSetBuilder
         */
        @Nullable
        protected ManagedDeviceAppDiagnosticsParameterSetBuilder(){}
        /**
         * Buils the resulting body object to be passed to the request
         * @return the body object to pass to the request
         */
        @Nonnull
        public ManagedDeviceAppDiagnosticsParameterSet build() {
            return new ManagedDeviceAppDiagnosticsParameterSet(this);
        }
    }
    /**
     * Gets the functions options from the properties that have been set
     * @return a list of function options for the request
     */
    @Nonnull
    public java.util.List<com.microsoft.graph.options.FunctionOption> getFunctionOptions() {
        final ArrayList<com.microsoft.graph.options.FunctionOption> result = new ArrayList<>();
        if(this.upn != null) {
            result.add(new com.microsoft.graph.options.FunctionOption("upn", upn));
        }
        return result;
    }
}
